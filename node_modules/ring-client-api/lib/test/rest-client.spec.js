...
const email = 'some@one.com', password = 'abc123!', phone = '+1xxxxxxxx89', twoFactorAuthCode = '123456', hardwareIdPromise = (0, util_1.getHardwareId)(), accessToken = 'ey__accees_token', secondAccessToken = 'ey__second_accees_token', refreshToken = 'ey__refresh_token', secondRefreshToken = 'ey__second_refresh_token', thirdRefreshToken = 'ey__third_refresh_token', server = (0, node_1.setupServer)(msw_1.http.post('https://oauth.ring.com/oauth/token', async ({ request: req }) => {
    ...
    if (req.headers.get('2fa-support') !== 'true' ||
        ...
        return msw_1.HttpResponse.json({
            code: 1,
            error: 'Invalid auth headers: ' + JSON.stringify(req.headers, null, 2),
        }, { status: 400 });
    }
...
}), msw_1.http.post('https://api.ring.com/clients_api/session', async ({ request: req }) => {
    const authHeader = req.headers.get('Authorization');
    if (authHeader !== `Bearer ${accessToken}` &&
        authHeader !== `Bearer ${secondAccessToken}`) {
        // Invalid access token used
        return msw_1.HttpResponse.json({}, { status: 401 });
    }
    const body = await req.json();
    if (body.device.hardware_id !== (await (0, util_1.getHardwareId)()) ||
        ...
        return msw_1.HttpResponse.text('Bad session request: ' + JSON.stringify(body, null, 2), { status: 400 });
    }
...
describe('fetch', () => {
    ...
    beforeEach(() => {
        ...
        server.use(msw_1.http.get('https://api.ring.com/clients_api/some_endpoint', ({ request: req }) => {
            const authHeader = req.headers.get('Authorization');
